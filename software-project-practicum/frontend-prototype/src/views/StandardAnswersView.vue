<template>
  <section class="view" v-if="taskStore.currentTask">
    ...
    <section class="panel answer-status-card">
      <div class="answer-status-card__main">
        ...
        <div class="summary-item">
          <span>当前题目状态</span>
          <strong>{{ hasQuestion ? "已配置" : "未配置" }}</strong>
        </div>
        <div class="summary-item">
          <span>标准答案状态</span>
          <strong>{{ hasStandardAnswer ? "已上传" : "未上传" }}</strong>
        </div>
        <div class="summary-item">
          <span>已保存版本数</span>
    ...
    <div class="answer-management-grid">
      <section class="panel answer-upload-card">
        ...
        <div class="file-meta-row">
          ...
          <span v-if="selectedFile">{{ configStore.formatFileSize(selectedFile.size) }}</span>
        </div>
      </section>

      <aside class="panel answer-status-card">
        <div class="panel__header">
          <h3>当前状态</h3>
        </div>
        <div class="summary-grid">
          <div class="summary-item"><span>题目定义</span><strong>{{ hasQuestion ? "已配置" : "未配置" }}</strong></div>
          <div class="summary-item"><span>标准答案版本</span><strong>{{ configStore.standardAnswers.length }} 个</strong></div>
    ...
    <section class="panel answer-upload-card">
      <div class="panel__header panel__header--stack">
        ...
        <button class="action-button action-button--ghost" :disabled="configStore.saving || !hasQuestion" @click="saveAnswer">
          保存为文本版本
        </button>
      </div>
      <label class="field">
        <span>标准答案文本</span>
        <textarea v-model="configStore.standardAnswerDraft" class="field__input field__input--textarea" rows="8" />
      </label>
    </section>
    ...
    <section class="panel">
      ...
      <div v-if="configStore.standardAnswers.length > 0" class="answer-version-list">
        <article v-for="item in configStore.standardAnswers" :key="item.id" class="answer-version-card">
          <div class="config-record-card__top">
            <div>
              <div class="rubric-card__title">{{ versionTitle(item) }}</div>
              <div class="rubric-card__meta">
                版本 {{ configStore.formatStandardAnswerVersion(item) }} · 保存时间 {{ item.created_at ?? item.updated_at ?? "暂无" }} · 状态 已保存
              </div>
            </div>
          ...
          <p class="answer-preview" :class="{ 'answer-preview--expanded': expandedIds.has(String(item.id)) }">
            {{ expandedIds.has(String(item.id)) ? fullAnswerText(item) : configStore.formatStandardAnswerSummary(item) }}
          </p>
        </article>
      </div>
